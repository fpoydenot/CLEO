...
#include "superdrops/collisions/fpdiffgravprob.hpp"

struct FPDiffGravCreateMicrophysics {
  inline MicrophysicalProcess auto operator()(const Config &config, const Timesteps &tsteps) const {
    const PairProbability auto prob = FPDiffGravProb();
...
int main(int argc, char *argv[]) {
  return generic_microphysics_main(argc, argv, FPDiffGravCreateMicrophysics{});
}
